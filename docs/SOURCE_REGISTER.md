...
| SEC EDGAR APIs | P | https://www.sec.gov/search-filings/edgar-application-programming-interfaces | `submissions` is current entity metadata/filing history; `companyfacts` provides facts but not per-fact acceptance; bulk ZIPs refresh nightly | 2026-07-18 | Before Phase 0.3/1.5 |
| SEC automated-access policy | P | https://www.sec.gov/about/developer-resources | Identify the client, throttle, cache and back off; implementation stays below the published request ceiling | 2026-07-18 | Before every deployment environment |
